...
//!
//! These models were trained against Codex, whose tool set is far narrower
//! than what fabro offers the other OpenAI models: a shell, `apply_patch`, and
//! `update_plan`. Codex has no file-read, file-write, grep, glob, or fetch
//! tool at all -- reading and searching go through the shell, and every write
//! goes through `apply_patch`. Offering 5.6 the tools it was not trained on
//! would advertise affordances its instructions never mention, so this profile
//! registers only what Codex does.
//!
...
impl Gpt56Profile {
    ...
    /// `summarizer` is accepted to keep one shape across profiles for
    /// [`super::AgentProfileBuilder`], and deliberately unused: it only feeds
    /// `web_fetch`, which this profile does not register.
    pub(crate) fn with_native_tools(
        model: impl Into<String>,
        options: &NativeToolOptions,
        _summarizer: Option<WebFetchSummarizer>,
    ) -> Self {
...
- Use `apply_patch` to edit files, not `cat`, heredocs, or other shell write tricks.
    ...
    RegisteredTool {
        ...
        executor:   Arc::new(move |args, ctx| {
            Box::pin(async move {
                ...
                let streaming =
                    tools::execute_shell_command(&ctx, command, timeout_ms, workdir).await?;

                let text = tools::render_shell_result(&streaming);
                let is_success = streaming.result.is_success();
                tools::emit_shell_process_completed(&ctx, streaming).await;
...
mod tests {
    ...
    fn web_search_is_registered_only_when_a_key_is_configured() {
        ...
        let searching = Gpt56Profile::with_native_tools("gpt-5.6-sol", &options, None);
    ...
    fn provider_prompt_uses_catalog_display_name() {
        let direct = Gpt56Profile::new("gpt-5.6-sol").with_catalog(test_catalog());
        ...
        let gateway = Gpt56Profile::new("gpt-5.6-sol")
            ...
            .with_catalog(catalog_with_openrouter());
    ...
    fn catalog_reports_the_5_6_context_window() {
        let profile = Gpt56Profile::new("gpt-5.6-sol").with_catalog(test_catalog());
