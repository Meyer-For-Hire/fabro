    ...
    R: DbRead + Sync,
{
    ...
    // Seek to the page cursor and decode only the requested page. Zero-padded
    // sequence keys scan in seq order, so no post-scan sort is needed.
    let mut iter = db.scan(keys::run_events_range(run_id, start_seq)).await?;
