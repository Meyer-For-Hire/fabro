...
Two profiles are selected per model rather than per provider, because they follow the model wherever it is served: `kimi` for Kimi models, and `gpt56` for the GPT-5.6 models (Sol, Terra, Luna). The `gpt56` profile registers only the tools Codex gives those models — a shell, `apply_patch`, and `update_plan` — because they were not trained with fabro's file-read, search, or fetch tools.
