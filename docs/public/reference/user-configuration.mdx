...
| `agent_profile` | `"anthropic"` \| `"openai"` \| `"gemini"` \| `"kimi"` \| `"gpt56"` | derived from `adapter` | Agent profile used for tool registration, project memory, CLI/ACP command selection, and native session routing. Override only when a provider needs profile behavior different from its adapter. |
...
| `agent_profile` | `"anthropic"` \| `"openai"` \| `"gemini"` \| `"kimi"` \| `"gpt56"` | provider profile | Agent profile override for this model. Model overrides take precedence over provider overrides — this is how Kimi and GPT-5.6 models keep their profile on every gateway that serves them. |
